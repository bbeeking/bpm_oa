...
<!-- 顶部导航栏 top start -->
 ...
 添加项目
                    ...
                    <form action="project_post.php?<?php if($pid>0) { ?>type=edit<?php } else { ?>type=add<?php } ?>" enctype="multipart/form-data" method="post" class="form-horizontal" onsubmit="return chksubmit();">
                    
                    <div class="row-fluid">
<div class="span12">
<h3 class="heading"><?php if($pid>0) { ?><?php echo L("修改项目");?><?php } else { ?><?php echo L("添加项目");?><?php } ?></h3>
<div class="row-fluid">
...
<div class="controls">
<?php echo get_section('parent_pid',$project,$projectinfo['parent_pid'],$ary_first=array(0=>L("请选择")));?>
<span class="help-block"><?php echo L("添加一级项目无需选择");?></span>
</div>
</div>
<div class="control-group formSep">
<label for="project_name" class="control-label"><span class="f_req">*</span>项目名称</label>
<div class="controls">
<input type="text" maxlength="30" name="project_name" id="project_name" value="<?php echo $projectinfo['project_name'];?>" onblur="if(this.value!=''){$('#projectnamemsg').html('');}" />
...
<div class="controls">
<input type="text" maxlength="3" name="locality" id="locality" value="<?php echo $projectinfo['locality'];?>" onblur="if(this.value!=''){$('#localitymsg').html('');}" />
...
<div class="controls">
<?php echo get_section('priority',$priorityStatusAry,$projectinfo['priority'],$ary_first=array(0=>L("请选择")));?>
</div>
</div>
<div class="control-group formSep">
<label for="status" class="control-label"><span class="f_req">*</span>状态</label>
<div class="controls">
<?php echo get_section('status',$jobStatus,$projectinfo['status'],$ary_first=array(0=>L("请选择")));?>
</div>
</div>
<div class="control-group formSep">
<label for="color" class="control-label">标记颜色</label>
<div class="controls">
<input type="text" maxlength="10" name="color" id="color" value="<?php echo $projectinfo['color'];?>"  />
</div>
</div>
<div class="control-group">
<div class="controls">
<input type="hidden" name="pid" id="pid" value="<?php echo $pid;?>" style="display:none;" />
<button class="btn btn-gebo" type="submit" name="submit" id="submit"><?php echo L("提交");?></button>
<input type="button" name="go_url" value="<?php echo L("返回");?>" onclick="window.location.href='task_project_manager.php';" />
...
$('#projectamemsg').html('<?php echo L("项目名不能为空！");?>');
$('#project_name').focus();
return false;
}
else{
//如果是添加则进入检测，如果修改则跳过
<?php if($pid<=0) { ?>
$('#projectnamemsg').html('<?php echo L("正在检测，请稍后...");?>');
var ac_str = GetRequest('task_ajax.php?type=project&project_name='+encodeURI(project_name));
if(ac_str == '2') {
$('#projectnamemsg').html('');
}
else if(ac_str == '1') {
$('#projectnamemsg').html('<?php echo L("项目名称已存在，请重新填写！");?>');
$('#project_name').focus();
return false;
}
else {
$('#project_namemsg').html('<?php echo L("程序可能出现异常，请联系管理员。");?>');
return false;
}
<?php } ?>
}
