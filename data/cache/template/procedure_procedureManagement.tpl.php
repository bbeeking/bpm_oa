                    ...
                    <table class="table table-striped table-bordered  mediaTable" id="dt_d">
                        <thead>
                            <tr>
                                ...
                                <th class="optional"><?php echo L("关联流程id");?></th>
                                <th class="optional"><?php echo L("案例名称");?></th>
                                <th class="optional"><?php echo L("表单流转");?></th>
                                <th class="optional"><?php echo L("当前节点");?></th>
                                <th class="optional"><?php echo L("指派");?></th>
                                <th class="optional"><?php echo L("状态");?></th>
                                <th class="optional"><?php echo L("业务创建");?></th>
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td><?php echo $val['form_circulation_sequence'];?></td>
                                <td><?php echo $val['current_location'];?></td>
                                <td><?php echo $val['current_executor'];?></td>
                                <td><?php echo $procedureStatusConfig[$val['status']];?></td>
                                <td><?php echo $realnameAry[$val['builder']];?></td>
                                <td><?php echo date('Y-m-d',$val['create_time']);?></td>
                                <td>
                                    <a href="#" onclick="javascript:window.location = 'flowAdd.php?id=<?php echo $val['id'];?>'">查看进度</a> |
                                    <a href="#" onclick="javascript:window.location = 'flowAdd.php?id=<?php echo $val['id'];?>'">催办</a> |

                                    <?php if($val['status'] == 1) { ?>
                                        <a href="#" onclick="recordStop(<?php echo $val['id'];?>,'<?php echo $val['case_name'];?>',2);">挂起</a> |
                                    <?php } elseif ($val['status'] == 2) { ?>
                                        <a href="#" onclick="recordStop(<?php echo $val['id'];?>,'<?php echo $val['case_name'];?>',1);">启用</a> |
                                    <?php } ?>

                                    <a href="#" onclick="recordDel(<?php echo $val['id'];?>,'<?php echo $val['case_name'];?>');">删除</a>
                                </td>
                            </tr>
...
<?php } ?>
    ...
    <script type="text/javascript">
        ...
        function recordDel(id,name){
            if(confirm('<?php echo L("删除前建议暂停该流程案例，是否确认删除流程案例：");?>'+name)) {
                //return true;
        ...
        function recordStop(id,name,status){
            //暂停
            if(status == ''){
                if(confirm('<?php echo L("暂停后该流程案例后所有流转操作都将停止直至恢复，是否确认暂停流程案例：");?>'+name)) {
                    //return true;
