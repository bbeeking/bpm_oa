<?php
 require_once('auth.php'); if (User::check_cookie() === true) { if (isset($_GET['logout'])) { User::logout(); } } $error = false; $login = ""; if (isset($_POST["username"])) { $username_in = $_POST["username"]; if (isset($_POST["password"])) { $password_in = $_POST["password"]; } else { $error = "Incorrect registration information"; } if (User::check_user($username_in, $password_in) == true) { User::login($username_in, $password_in); header('Location: admin.php'); die(); } else { $error = "Incorrect login/password"; } } ?>
