...
<div id="maincontainer" class="clearfix">
            ...
            <form action="" class="form-horizontal well" enctype="multipart/form-data"  name="FormPage" id="FormPage"  method="post" >
                <fieldset>
                    ...
                    <div class="control-group">
                        ...
                        <div class="controls">
                            <input type="text" class="dfinput"  name="name" id="name" value="{$result['process_title']}" onchange="checkboxControl();" />
                    ...
                    <div class="control-group">
                        ...
                        <div class="controls">
                            <input type="text" class="dfinput"  name="type" id="type" value="{$result['type']}"  />
                    ...
                    <div class="control-group">
                        ...
                        <div class="controls">
                            {get_section('category',$elementTypeConfigAry,$result['category'],$ary_first=array(0=>L("请选择")),'category','','',$etc='data-placeholder="请选择分类..."')}
                    ...
                    <div class="control-group">
                        ...
                        <div class="controls">
                            {get_section('status',$enabledConfigAry,$result['status'],$ary_first=array(0=>L("请选择")),'status','','',$etc='data-placeholder="是否开启..."')}
                    ...
                    <div class="control-group">
                        ...
                        <div class="controls">
                            {get_section('debug',$enabledConfigAry,$result['debug'],$ary_first=array(0=>L("请选择")),'debug','','',$etc='data-placeholder="是否调试信息..."')}
                    ...
                    <div class="control-group">
                        ...
                        <div class="controls">
                            <textarea name="config_data" id="config_data" class="auto_expand span6" cols="1" rows="4">{$result['config_data']}</textarea>
                        </div>
                    </div>
                    <div class="control-group">
                        <label class="control-label">节点的属性：</label><font color="#ff4500">（高级：关联表，gateway，event，action等）</font>
                        <div class="controls">
                            <textarea name="node_property_config" id="node_property_config" class="auto_expand span6" cols="1" rows="4">{$result['node_property_config']}</textarea>
                        </div>
                    </div>
                    <div class="control-group">
                        <div class="controls">
                            ...
                            {if $_GET["id"]>0}
                            <a href="../system/add_menu.php?type=flowGo&flowId={$_GET["id"]}">
                                <input class="btn-info" type="button"  class="btn" value="+加入菜单" /></a>
                            <a href="../system/add_menu.php?type=flowData&flowId={$_GET["id"]}">
    ...
    <script type="text/javascript">
        function chksubmit()
        {
            ...
            if(name == '') {
                $('#nameMsg').html('{L("请填写流程名称！")}');
                $('#name').focus();
                return false;
            }
            if(status == '0' ) {
                $('#statusMsg').html('{L("请填写开启状态！")}');
                $('#formElementSign').focus();
                return false;
            }
            if(debug == '0') {
                $('#debugMsg').html('{L("请填写调试状态！")}');
                $('#debug').focus();
                return false;
            }
