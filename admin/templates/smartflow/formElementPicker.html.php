...
<form id="form1" name="form1" method="post" action="" class="myForm">
  ...
  <div class="form_content">
      <table class="table table-striped table-bordered mediaTable" id="dt_d">
        <thead>
        <tr>
          <th class="optional">{L("Dynaforms 动态表名")}</th>
          <th class="optional">{L("Variable 字段名")}</th>
          <th class="optional">{L("param")}</th>
          <th class="optional">{L("Sign")}</th>
          <th class="optional">{L("Type 类型")}</th>
        </tr>
        </thead>
        <tbody>
...
<!--            <tr ondblclick="alert($(this).children().eq(2).text());">-->
            <tr ondblclick="window.opener.document.getElementById('{$tagName}').value+=$(this).children().eq(2).text();window.close();">
              <td>{$key}</td>
              <td>{$vv['name']}</td>
              <td>{$vv['param']}</td>
              <td>{$vv['sign']}</td>
