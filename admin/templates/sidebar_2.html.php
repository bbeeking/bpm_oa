...
<!--										<i class="{if isset($sidebarIconGlyphs[$key])}{$sidebarIconGlyphs[$key]}{else}icon-folder-close{/if}"></i> {$depth1['m_name']}-->
										<i class="{if !empty($depth1['icon'])}{$depth1['icon']}{else}icon-folder-close{/if}"></i> {$depth1['m_name']}
									</a>
								</div>
								...
								{/if}
									...
									<div class="accordion-inner">
										<ul class="nav nav-list">
											...
											<li>
											{/if}
												<a style="cursor: pointer;" onclick="set_click_module_sign('{$depth2['module_sign']}','{DAEM_PATH}admin/{$depth2['m_url']}');">
													<i class="{if !empty($depth2['icon'])}{$depth2['icon']}{/if}"></i>{$depth2['m_name']}
												</a></li>
