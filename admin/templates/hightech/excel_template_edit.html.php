...
<!--                            <span class="pull-right label label-important">{$dataAryCount} Orders</span>-->
                        ...
                        <div class="control-group">
                            ...
                            <div class="controls">
                                <input type="text" id="templateName" name="templateName" value="{$fileName}" />
                            </div>
                        </div>
                        <div class="control-group">
                            <label class="control-label">模板标识：</label>
                            <div class="controls">
                                <input type="text" id="templateSign" name="templateSign" value="{$sign}" /><font style="font-size: 12px;color: red;">（纯字母不含特殊字符和空格）</font>
                            </div>
                        </div>
    ...
    <!--                    <table border="1px;">-->
                            ...
                            <?php
 for($i=0;$i<count($dataAry);$i++) { echo "<tr>"; for($j=0;$j<count($dataAry[$i]);$j++) { if($dataAry[$i][$j]["type"]!= 'title') { echo "<td>".$dataAry[$i][$j]["name"]."</td>"; if(count($dataAry[$i]) == 1) { echo "<td colspan='".($maxCountNum-1)."'>".get_return_section($dataAry[$i][$j]["param"],$dataTypeConfigAry,$dataAry[$i][$j]["type"],'',$dataAry[$i][$j]["param"],'onchange="change_section(this.options[this.options.selectedIndex].value,\''.$dataAry[$i][$j]["param"].'_id\')"',''). "&nbsp;<a onclick=\"property_section('".$dataAry[$i][$j]["param"]."_property');\">属性</a>
                                                ...
                                                <span style=\"display:none;\"  id=\"".$dataAry[$i][$j]["param"]."_property\">".get_return_section($dataAry[$i][$j]["param"].'_element_property',$elementPropertyExtraAry,'',$ary_first=array(0=>L("请选择")),$dataAry[$i][$j]["param"].'_element_property','','',$etc='data-placeholder="请选择单元格属性..."')."</span></td>"; } else { echo "<td>".get_return_section($dataAry[$i][$j]["param"],$dataTypeConfigAry,$dataAry[$i][$j]["type"],'',$dataAry[$i][$j]["param"],'onchange="change_section(this.options[this.options.selectedIndex].value,\''.$dataAry[$i][$j]["param"].'_id\')"',''). "&nbsp;<a onclick=\"property_section('".$dataAry[$i][$j]["param"]."_property');\">属性</a>
                                                <span style=\"display:none;\"  id=\"".$dataAry[$i][$j]["param"]."_id\">".get_return_section($dataAry[$i][$j]["param"].'_element',$formElementAry,'','',$dataAry[$i][$j]["param"].'_element','','',$etc='data-placeholder="请选择单元格标识..."')."</span>
                                                <span style=\"display:none;\"  id=\"".$dataAry[$i][$j]["param"]."_property\">".get_return_section($dataAry[$i][$j]["param"].'_element_property',$elementPropertyExtraAry,'',$ary_first=array(0=>L("请选择")),$dataAry[$i][$j]["param"].'_element_property','','',$etc='data-placeholder="请选择单元格属性..."')."</span></td>"; } } else { echo "<td colspan='".$maxCountNum."' style='text-align:center;font-size:18px;font-weight:600'>".$dataAry[$i][$j]["name"]."</td>"; } } echo "</tr>"; } ?>
                        ...
                        <div class="control-group">
                            <div class="controls" style="float: right;">
                                ...
                                <input type="hidden" name="fileBasicPath" id="fileBasicPath" value="{$fileBasicPath}">
