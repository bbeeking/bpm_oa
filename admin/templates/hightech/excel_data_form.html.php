                ...
                <fieldset>
                    ...
                    <div class="control-group">
                        <div class="controls">
                            ...
                            <a href="../system/add_menu.php?templateSign={$_GET["templateSign"]}"><input class="btn btn-info" type="button"  value="+加入菜单" /></a>
                            <!--
                            <a href="./excelBI.php?templateDel=1&templateSign={$_GET["templateSign"]}"><input class="btn btn-danger" type="button" value="删除模板" /></a>
                            -->
                            <a href="javascript:void(0)" class="del_confirm" title="Delete" id="{$_GET["templateSign"]}"><input class="btn btn-danger" type="button" value="删除模板" /></a>
                ...
                <div class="row-fluid">
                    <div class="span12">
                        ...
                        <div class="heading clearfix">
                            ...
                            <!--
                            <a href="../system/add_menu.php?templateSign={$_GET["templateSign"]}"><input class="btn-info btn-mini" type="button"  class="btn" value="+加入菜单" /></a>
                            -->
 ...
 for($i=0;$i<count($dataAry);$i++) { echo "<tr>"; for($j=0;$j<count($dataAry[$i]);$j++) { $isNeed = ($dataAry[$i][$j]["isNeed"] == '1') ? " <span class=\"f_req\">*</span>" : ""; $value = ($_GET["id"] > 0) ? $data[$dataAry[$i][$j]["param"]] : $dataAry[$i][$j]['propertyInfo']['defaultValue']; if(in_array($dataAry[$i][$j]["type"],array("file","image"))) { $value = json_decode($value,true); } if($dataAry[$i][$j]["type"]!= 'title') { echo "<td>".$dataAry[$i][$j]["name"].$isNeed."</td>"; if(count($dataAry[$i]) == 1) { echo "<td colspan='".($maxCountNum-1)."'>"; if($dataAry[$i][$j]["type"] == "text") { echo "<input type=\"text\" id=\"".$dataAry[$i][$j]["param"]."\" name=\"".$dataAry[$i][$j]["param"]."\" value=\"".$value."\" />"; } elseif($dataAry[$i][$j]["type"] == "time") { echo "<div class=\"input-append date form_time\" data-date=\"\" data-date-format=\"hh:ii\" data-link-field=\"dtp_input3\" data-link-format=\"hh:ii\">
                        ...
                        <div class="control-group">
                            <div class="controls" style="float: right;">
                                ...
                                <input name="submit2" class="btn-success" type="button" onclick="submit_template();" class="btn" value="{$actName}" />
...
//            alert(templateSign);return;
        ...
        function delconfirm(templateSign){
            smoke.confirm('{L("删除表单模板将同时删除关联的表记录信息，该表单关联的流程或将无法使用，是否确认删除?")}',function(e){
                if (e){
                    //回调处理，执行跳转动作
